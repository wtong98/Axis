...
class cstudent extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('student', 'student', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// IMSAid
		$this->IMSAid = new cField('student', 'student', 'x_IMSAid', 'IMSAid', '`IMSAid`', '`IMSAid`', 200, -1, FALSE, '`IMSAid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->IMSAid->Sortable = TRUE; // Allow sort
		$this->fields['IMSAid'] = &$this->IMSAid;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->IMSAid->EditValue = $this->IMSAid->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->IMSAid->Exportable) $Doc->ExportCaption($this->IMSAid);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->IMSAid->Exportable) $Doc->ExportField($this->IMSAid);
