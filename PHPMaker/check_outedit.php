...
$check_out_edit = NULL; // Initialize page object first

class ccheck_out_edit extends ccheck_out {
	...
	// Page object name
	var $PageObjName = 'check_out_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("check_outlist.php"); // Invalid key, return to list
		}
...
// Create page object
if (!isset($check_out_edit)) $check_out_edit = new ccheck_out_edit();

// Page init
$check_out_edit->Page_Init();

// Page main
$check_out_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$check_out_edit->Page_Render();
?>
...
var CurrentForm = fcheck_outedit = new ew_Form("fcheck_outedit", "edit");

// Validate form
fcheck_outedit.Validate = function() {
...
fcheck_outedit.Form_CustomValidate = 
...
fcheck_outedit.ValidateRequired = true;
<?php } else { ?>
fcheck_outedit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $check_out_edit->ShowPageHeader(); ?>
<?php
$check_out_edit->ShowMessage();
?>
<form name="fcheck_outedit" id="fcheck_outedit" class="<?php echo $check_out_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($check_out_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $check_out_edit->Token ?>">
<?php } ?>
...
<?php if ($check_out->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_check_out_id" class="col-sm-2 control-label ewLabel"><?php echo $check_out->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $check_out->id->CellAttributes() ?>>
...
<input type="hidden" data-table="check_out" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($check_out->id->CurrentValue) ?>">
<?php echo $check_out->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $check_out_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
