...
class cbook extends cTable {
	...
	function __construct() {
		...
		// book_id
		$this->book_id = new cField('book', 'book', 'x_book_id', 'book_id', '`book_id`', '`book_id`', 19, -1, FALSE, '`book_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// IMSA_id
		$this->IMSA_id = new cField('book', 'book', 'x_IMSA_id', 'IMSA_id', '`IMSA_id`', '`IMSA_id`', 200, -1, FALSE, '`IMSA_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->IMSA_id->Sortable = TRUE; // Allow sort
		$this->fields['IMSA_id'] = &$this->IMSA_id;

		// title
		$this->title = new cField('book', 'book', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// ISBN
		$this->ISBN = new cField('book', 'book', 'x_ISBN', 'ISBN', '`ISBN`', '`ISBN`', 200, -1, FALSE, '`ISBN`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ISBN->Sortable = TRUE; // Allow sort
		$this->fields['ISBN'] = &$this->ISBN;

		// cost
		$this->cost = new cField('book', 'book', 'x_cost', 'cost', '`cost`', '`cost`', 200, -1, FALSE, '`cost`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cost->Sortable = TRUE; // Allow sort
		$this->fields['cost'] = &$this->cost;

		// fee
		$this->fee = new cField('book', 'book', 'x_fee', 'fee', '`fee`', '`fee`', 200, -1, FALSE, '`fee`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fee->Sortable = TRUE; // Allow sort
		$this->fields['fee'] = &$this->fee;

		// dept_id
		$this->dept_id = new cField('book', 'book', 'x_dept_id', 'dept_id', '`dept_id`', '`dept_id`', 3, -1, FALSE, '`dept_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->dept_id->Sortable = TRUE; // Allow sort
		$this->dept_id->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->dept_id->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["book_id"]))
				$arKeys[] = ew_StripSlashes($_POST["book_id"]);
			elseif (isset($_GET["book_id"]))
				$arKeys[] = ew_StripSlashes($_GET["book_id"]);
			else
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->fee->Exportable) $Doc->ExportCaption($this->fee);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->fee->Exportable) $Doc->ExportField($this->fee);
